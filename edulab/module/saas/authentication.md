...
This page explains how authentication works in the Tenanta module, how tenant admin access is isolated from central admin access, and how sessions are managed across domains.
...
|-------|--------|----------|-------|---------|
| `admin` | session | `admins` provider | `admins` | Tenanta admins + tenant admins |
...
1. **Admin guard** — Tenanta platform administrators log in here to access `/tenanta/admin/*`
...
|------------|-------------------|------------|
| Central Tenanta Admin | `NULL` | Tenanta admin panel (`/tenanta/admin/*`) |
| Tenant Admin | Non-null (tenant UUID) | Only their tenant's LMS admin panel |

### How Tenant Admins Are Blocked from the Tenanta Panel

Two layers of protection prevent tenant admins from accessing the central Tenanta admin panel:
...
#### Layer 2: EnsureCentralTenantaAdmin Middleware

All `/tenanta/admin/*` routes are protected by the `EnsureCentralTenantaAdmin` middleware:

1. **Primary check:** If the `admins` table has a `tenant_id` column and the logged-in admin's `tenant_id` is not null → reject
2. **Secondary check:** Consult the `TenantAdminRegistryService` (the `tenanta_tenant_admins` registry table) → reject if found
...
│   ├── Central admin (tenant_id = null) → redirect to Tenanta dashboard
...
└── Show login form
    ...
    └── POST /tenant/login
        ...
        │   ├── Auth succeeds + tenant_id is null → redirect to Tenanta dashboard ✓
